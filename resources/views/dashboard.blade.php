...
<head>
    ...
    <style>
        ...
        .device-col { white-space: normal !important; word-break: break-word; min-width: 250px; font-size: 0.85rem; color: #555; }
        .isp-col { min-width: 150px; }
        .map-link { font-size: 0.8rem; text-decoration: none; display: block; margin-top: 5px; }
            ...
            <table class="table table-hover align-middle border">
                <thead class="table-light">
                    <tr>
                        <th>IP Address</th>
                        <th>Location</th>
                        <th class="isp-col">Network (ISP)</th> 
                        ...
                        <td>
                            ...
                            @if($row->latitude && $row->longitude)
                                <a href="https://www.google.com/maps?q={{ $row->latitude }},{{ $row->longitude }}" 
                                   target="_blank" class="text-success fw-bold map-link">
                                   🎯 View on Map
                                </a>
                            @else
                                <small class="text-muted">(GPS Not Available)</small>
                            @endif
                        </td>
                        <td class="isp-col">
                            <span class="badge bg-info text-dark">{{ $row->isp ?? 'Unknown' }}</span>
                        </td> 
